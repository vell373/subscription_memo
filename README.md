...
- ✅ サブスクリプションサービスの追加・**編集**・削除
- ✅ 月額・年額の料金管理
- ✅ **換算金額表示**（月額⇔年額の自動換算）
- ✅ **スマート合計計算**（月額換算・年額換算の統合合計）
- ✅ **検索・並び替え機能**（サービス名検索、多様なソートオプション）
- ✅ **複数PC間でのデータ同期**（chrome.storage.sync対応）
- ✅ CSV形式でのデータエクスポート
...
- **検索**: テーブル上の検索ボックスでサービス名を絞り込み
- **並び替えオプション**:
  - 名前順（昇順/降順）
  - 月額金額順（昇順/降順）※全て月額換算で比較
  - 年額金額順（昇順/降順）※全て年額換算で比較
  - 月額のみ表示
  - 年額のみ表示

### 同期機能

- **同期設定**: 「複数PC間でデータを同期する」チェックボックで有効/無効を切り替え
- **自動データ移行**: 同期有効時にローカルデータを自動で同期ストレージに移行
- **手動同期**: 「今すぐ同期」ボタンで即座同期実行
- **同期状態表示**: 現在の同期状態をリアルタイムで表示

### データの管理

- **編集**: 各サービスの「編集」ボタンでサービス名、金額、周期を変更
